...
function tradePortionOfAsset(tradePortion){
  ...
  this.valueOfAsset = (tradePortion.commodity.value * tradePortion.percentage / 100)
  ...
  owners.forEach(function(owner, index, arr){
    ...
    if(owner.trader== tradePortion.oldOwner){
      ...
      if(owner.percentage == 0)
        this.removeOldOwner = true;
      //oldowner share less than required, we will throw error
      if(owner.percentage < 0 )
        this.oldOwnerPercentageNotEnough = true;
    }
  ...
  //remove old owner as he has no more percentage
  if(this.removeOldOwner)
    owners.splice(this.oldOwnerIndex, 1);
  ...
  getParticipantRegistry('org.example.mynetwork.Trader').then(function (participantRegistry) {
      tradePortion.oldOwner.balance = tradePortion.oldOwner.balance + this.valueOfAsset
      tradePortion.newOwner.balance = tradePortion.newOwner.balance - this.valueOfAsset
